 ...
 * properties of the stand sh capsis.sh -p script fireparadox.userscript.ScriptFP_ifndata
 * 
 * @author F. de Coligny and FP- june 2010
 * 
 */
public class ScriptFP_ifndata {
	...
	private static class Parameters {
		...
		public Parameters (int ageTot, double hDom50, double stemDensity, double gibbs,
				double liveNeedleMoistureContent, double liveTwigMoistureContent, double deadTwigMoistureContent,
				int layerSetAge, double fertility, double shrubPercentage, double shrubCharacteristicSize,
				double herbPercentage, double herbCharacteristicSize, double liveMC, double deadMC, double sceneSize) {
			// tree parameters
			this.ageTot = ageTot;
			this.hDom50 = hDom50;
	...
	public static void main (String[] args) throws Throwable {
		...
		String name = "ScriptFP_ifndata";
		// Collection<Parameters> parameterSet = new ArrayList<Parameters>();
		// parameterSet.add(new Parameters(50, 12d, 200d, 1000d, 100d, 80d, 10d, 15, 2, 90d, 4d,
		// 20d,1d,70d,10d, 200d));
		...
		String outDir = PathManager.getDir ("data") // .../capsis4/data
				+ File.separator + ".." + File.separator + ".." + File.separator + "Documents"
				+ File.separator
				+ "capsisdata" + File.separator + "scriptoutput" + File.separator + name;
		...
		ParamParser pp = new ParamParser (outDir + File.separator + "param2.txt");
		...
		for (Parameters p : parameterSet) {
			...
			initStand
					.addTreeGroup2 (speciesName, currentPoly, hDom50, ageTot, stemDensity, gHaETFactor, "HARDCORE", gibbs, liveNeedleMoistureContent, deadTwigMoistureContent, liveTwigMoistureContent);
			...
			FmLayerSet ls = new FmLayerSet (/* initStand, */ initStand.maxId, currentPoly, fertility, shrubPercentage,
					shrubCharacteristicSize, herbPercentage, herbCharacteristicSize, liveMC, deadMC);
			// initial evolution
			...
			// Write in the file when needed
			writeReportLine (p, 0d, out, root, step);
			...
			if (thinning) {
				...
				NrgThinner2 t = new NrgThinner2 (distCriterion, minDist, thinningCriterion, martellingDist);

				step = s.runIntervener (t, root);
				writeReportLine (p, minDist, out, root, step);

				minDist = 1;
				t = new NrgThinner2 (distCriterion, minDist, thinningCriterion, martellingDist);
...
//				FiretecSingleFamily export = new FiretecSingleFamily (2d, 2d, 15d, 41, 0.1, minX, minY, maxX - minX,
//						maxY - minY, thinTwigsIncluded, overlappingPermitted, polygonResolution,
//						(FmStand) step.getScene (), format);
//				export.save (dirName);
				
			}
			Engine.getInstance ().processSaveAsProject (s.getProject (), outFileName + ".prj");
	 ...
	 */
	static private void writeReportHeader (String name, BufferedWriter out) throws Exception {
		// out.write("Report of fireparadox.userscript." + name + " run at "
		// + new Date());
		// out.newLine();
		// out.newLine();
		out.write ("age	hdom50	N	domainSize	iNcomp	icover	ilai	icover2	igha	ihmean	iload	minDist	Ncomp	cover	lai	cover2	gha	hmean	load");
		out.newLine ();
	 ...
	 */
	static private void writeReportLine (Parameters param, double minDist, BufferedWriter out, Step root, Step step)
			throws Exception {
		...
		double[] properties = FmStand.calcMultiCov (stand.getTrees (), null, stand.getPlot (), 0d, null, 
		...
		for (Tree tree : stand.getTrees ()) {
			cover2 += Math.PI * Math.pow (((FiPlant) tree).getCrownRadius (), 2d);
		...
		stand = (FmStand) step.getScene ();
		n = stand.getTrees ().size ();
		stemDensity = n / (param.sceneSize * param.sceneSize) * 10000d;
		b.append (stemDensity); // Ncomp
		b.append ('\t');

		properties = FmStand.calcMultiCov (stand.getTrees (), null, stand.getPlot (), 0d, null, 
				m.particleNames); // fc-2.2.2015 particleNames
		cover = properties[0];
		...
		for (Tree tree : stand.getTrees ()) {
			...
			gha += Math.PI * tree.getDbh () * tree.getDbh () / 4d * 0.0001;
			hmean += tree.getHeight ();
			load += ((FiPlant) tree).getTotalThinMass ();
		}
		cover2 *= 1d / (param.sceneSize * param.sceneSize) * 100d;
		gha *= 1d / (param.sceneSize * param.sceneSize) * 10000d;
		hmean *= 1d / n;
