 ...
 */
public class FmPatternMap implements Cloneable, Serializable {

	public static final FmPattern DEFAULT_PATTERN= new FmPattern ();

	//A list of known patterns
	private FmPatternList patternList;
	...
	public FmPatternMap (FmPatternList patternList) {
		super ();
	...
	public FmPattern getPattern (String criteria)  {
		String patternId = this.get (criteria);
		if(patternId == null) {
			return FmPatternMap.DEFAULT_PATTERN;
		} else {
			FmPattern fPattern=patternList.get (patternId);
			if(fPattern == null) {
				return FmPatternMap.DEFAULT_PATTERN;
			} else {
				return fPattern;
	...
	public void checkCoherence (String selectSpecie, double heightMin, double heightMax, String selectEnvi) throws Exception {
		...
		for (Object o : entry) {
			...
			String [] key = FmPatternMap.parseCriteria (criteria);
	...
	public void load () throws Exception{
		try {
			...
			FmPatternMapLoader loaderMap;
			loaderMap = new FmPatternMapLoader (fileName, false);

			patternMap.putAll (loaderMap.getPatternMap ());


			loaderMap = new FmPatternMapLoader (fileAdminName, true);
	...
	public void save () throws Exception{
		...
		try {
			...
			FmPatternMapLoader loaderMap;
			if(admin) {
				loaderMap = new FmPatternMapLoader (this.adminPatternMap);
				loaderMap.save (fileAdminName);
			}
			loaderMap = new FmPatternMapLoader (this.patternMap);
...
//			= 1 : check on the specie and the nearest interval
	...
	public FmPattern findPattern(String speciesSubject, double height,
			int precisionLevel) {
		...
		FmPattern fpTemp = null; //save the pattern of the nearest interval found
	...
	public FmPattern findAlwaysPattern (FmPlant subject) {


		String speciesSubject = subject.getSpecies ().getName ();

		int level = 3;
		FmPattern foundPattern = null;
		...
		if(foundPattern == null) {//if always null
			...
			foundPattern = new FmPattern (subject); // default pattern based on crownprofile of the subject
		}
