 ...
 */
public class FmGeomMapLoader extends RecordSet {
	 ...
	 */
	public FmGeomMapLoader (String fileName, boolean adminFile) throws Exception {
		super ();
	 ...
	 */
	public FmGeomMapLoader (TreeMap<String,String> patternMap) throws Exception {
		super ();
	...
	private void createRecordSet (TreeMap<String,String> patternMap) throws Exception {
		...
		for (Map.Entry e: setEntry) {
			...
			String [] criteriaTokens = FmGeomMap.parseCriteria (criteria);
		...
		for (Iterator i = this.iterator (); i.hasNext ();) {
			...
			if (record instanceof KeyRecord) {
				...
				if (r.hasKey ("species")) {
					...
					if(currentSpecie!="") {
						String currentKey = FmGeomMap.createCriteria (currentSpecie,currentHMin,currentHMax,currentEnvironment,adminFile);
