...
/**	FiPlantResearchForm : research criterias for plants
*
*	@author I. Lecomte - march 2008
*/
public class FmPlantResearchForm extends JPanel implements ActionListener {
    ...
    private JComboBox crownDiameterCombo;			//crown diameter
    private Vector listCD = new Vector();
	...
	private LinkedHashMap<Long, FmDBPlant>  listAllPlant;
    private JTable resultTable;
    private FmPlantTableModel tableModel;

	/**	Constructor.
	*/
    public FmPlantResearchForm (FmModel _model, FmDBTeam _teamLogged, int _rightLevel, int _fuelType) {
		...
		tableModel = new  FmPlantTableModel (rightLevel, fuelType);
		tableModel.clear();
		...
		try {
			...
			Log.println (Log.ERROR, "FiPlantResearchForm ()", "error while opening data base", e);
		}
    ...
    public void createUI() {
		...
		col1.add (new JLabel(Translator.swap ("FiPlantResearchForm.team")));
		col1.add (teamCombo);
		ligCrit1.add (col1);
		...
		col2.add (new JLabel(Translator.swap ("FiPlantResearchForm.site")));
		col2.add (siteCombo);
		ligCrit1.add (col2);

		// 3 Species
    	JLabel lbSpecies = new JLabel(Translator.swap ("FiPlantResearchForm.species"));
		...
		// 4 Crown Height
    	JLabel lbCrownHeight = new JLabel(Translator.swap ("FiPlantResearchForm.height"));
		...
		// 5 Crown Diameter
		if (fuelType == 1) {
    		JLabel lbCrownDiameter = new JLabel(Translator.swap ("FiPlantResearchForm.crownDiameter"));
			ColumnPanel col5 = new ColumnPanel ();
			crownDiameterCombo.addActionListener(this);
			col5.add (lbCrownDiameter);
			col5.add (crownDiameterCombo);
			ligCrit1.add (col5);
		}
		...
		col6.add (new JLabel(Translator.swap ("FiPlantResearchForm.origin")));
		...
		col7.add (new JLabel(Translator.swap ("FiPlantResearchForm.valid")));
		col7.add (validCombo);
		ligCrit1.add (col7);
		...
		col8.add (new JLabel(Translator.swap ("FiPlantResearchForm.delete")));
		...
		Border bRes = BorderFactory.createTitledBorder (etchedRes, Translator.swap (
				"FiPlantResearchForm.resultBorder"));
	...
	public void loadEnums () {
		...
		listCD.add ("9:10");
		listCD.add ("10:50");
		crownDiameterCombo = new JComboBox(listCD);
...
/** Load database information (all)
	...
	public void loadAllData () {
		...
		try {
			...
			listAllPlant = new LinkedHashMap<Long, FmDBPlant> ();
			listAllPlant = bdCommunicator.getPlants (fuelType,  teamMap, siteMap, delete, validate);
			for (Iterator i = listAllPlant.keySet().iterator(); i.hasNext ();) {
				Object cle = i.next();
				FmDBPlant f = listAllPlant.get(cle);

				String teamName = " ";
				if (f.getTeam() != null) teamName = f.getTeam().getTeamCode();

				String siteName = " ";
				if (f.getSite() != null) siteName = f.getSite().getSiteCode();

				FmPlantTable pltb = 	new FmPlantTable (f.getPlantId(),
														teamName,
														siteName,
														f.getSpeciesName(),
														f.getHeight(),
				...
				if (!speciesList.contains (f.getSpeciesName())) {
					speciesList.add (f.getSpeciesName());
				}

			}

			//sort the plants results
			Collections.sort(sortedPlant);
			for (Iterator iter = sortedPlant.iterator(); iter.hasNext ();) {
				FmPlantTable pltb = (FmPlantTable) iter.next();
				Long cle = pltb.getId();
				FmDBPlant pl = listAllPlant.get(cle);
				tableModel.addPlant (pl);
			}
	...
	public void loadData (String newCriteria) {
		...
		//shape specific selection
		String diameter = "";

		if (fuelType == 1) {	//PLANT crown diameter selection
			if (crownDiameterCombo.getSelectedIndex () > 0) {
				diameter = crownDiameterCombo.getSelectedItem ().toString ();
				checkDiameter = true;
			}
		}
		...
		for (Iterator i = listAllPlant.keySet().iterator(); i.hasNext ();) {
			...
			if (checkTeam) 	 check1 = f.checkTeamCode (teamCode);
			if (checkSite) 	 check2 = f.checkSiteCode (siteCode);
			if (checkSpecie) check3 = f.checkSpeciesName (speciesName);
			if (checkHeight) check4 = f.checkHeight (height);
			if (checkDiameter) check5 = f.checkDiameter (diameter);
			if (checkOrigin) check6 = f.checkOrigin (origin);
			if (checkValid)  check7 = f.checkValidated (validated);
			if (checkDelete)  check8 = f.checkDeleted (deleted);
	...
	public FmDBPlant getPlant (int index) {
		return tableModel.getPlant(index);
	}

    /** return the plant store at this index in the displayed table
     */
	public long getPlantId (int index) {
		FmDBPlant plant =  tableModel.getPlant(index);
    ...
    /** return if the plant is validated or not
     */
	public boolean getValidated (int index) {
		FmDBPlant plant =  tableModel.getPlant(index);
		return plant.isValidated();
    ...
    /** Add a new plant with fuel editor
     */
	public void addPlantEntry (FmDBPlant plant) {
		listAllPlant.put (plant.getPlantId(), plant);
		tableModel.addPlant (plant);
	}

	/** remove fuel with fuel editor
	 */
	public void removePlantEntry (FmDBPlant plant) {
		listAllPlant.remove (plant);
		tableModel.removePlant (plant);
	}
	public LinkedHashMap<Long, FmDBPlant> getPlantMap () {
		return listAllPlant;
	}
}
