 ...
 * If ascending, Returns -1 if t1.getX () < t2.getX (),
 * returns +1 if t1.getX () > t2.getX (),
 * returns 0 otherwise (==).
 *
 * @author F. de Coligny - september 2007
 */
public class SpatializedXComparator implements Comparator {

	boolean ascending;


	public SpatializedXComparator (boolean a) {ascending = a;}
	
	public SpatializedXComparator () {this (true);}
	...
	public int compare (Object o1, Object o2) throws ClassCastException {
		...
		double d1 = ((Spatialized) o1).getX ();
		double d2 = ((Spatialized) o2).getX ();
		...
		if (d1 < d2) {
			...
			double d3 = ((Spatialized) o1).getY ();
			double d4 = ((Spatialized) o2).getY ();
			if (d3 < d4) {
				if(!ascending) return 1;		// for descending order
				else return -1;							// t1 < t2
			} else if  (d3 > d4) {
