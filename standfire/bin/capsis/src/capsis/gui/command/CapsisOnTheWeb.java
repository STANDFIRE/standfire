...
/**
 * Command Help | CapsisOnTheWeb.
 *
 * @author F. de Coligny - october 2000
 */
public class CapsisOnTheWeb implements Command {

	/**
	 * Constructor.
	 */
	public CapsisOnTheWeb () {}

	public int execute () {
		
		try {
			Helper.showPage (Settings.getProperty ("capsis.url", "http://capsis.cirad.fr"));
			
		} catch (Throwable e) {		// fc - 30.7.2004 - catch Errors in every command (for OutOfMemory)
			Log.println (Log.ERROR, "CapsisOnTheWeb.execute ()", "An Exception/Error occured", e);
