...
import capsis.gui.DockedPositioner;
import capsis.gui.MainFrame;
import capsis.gui.Pilot;
import capsis.gui.Positioner;

/**
 * Command Edit | DockedOrg.
 *
 * @author F. de Coligny - april 2003
 */
public class DockedOrg implements Command {
	...
	public int execute () {
		...
		try {
			...
			Pilot.setPositioner (new DockedPositioner (MainFrame.getInstance (), pos));
			
		} catch (Throwable e) {		// fc - 30.7.2004 - catch Errors in every command (for OutOfMemory)
			Log.println (Log.ERROR, "DockedOrg.execute ()", "An Exception/Error occured", e);
