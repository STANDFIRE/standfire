...
import capsis.gui.CapsisPreferences;
...
public class Options implements Command {
	...
	public int execute () {
		...
		try {
			CapsisPreferences dlg = new CapsisPreferences (MainFrame.getInstance ());
			dlg.dispose ();		// when all finished, destroys the dialog
			
		} catch (Throwable e) {		// fc - 30.7.2004 - catch Errors in every command (for OutOfMemory)
			Log.println (Log.ERROR, "Options.execute ()", "An Exception/Error occured", e);
