...
import capsis.gui.autoui.AutoDialog;
...
public class DefaultRelay extends Relay implements SketchLinkable {
	...
	private SketchLinker sketchLinker;
	...
	*/
	...
	/**	Return dialog to set initial parameters 
	*	Used by default package.gui.PrefixInitialDialog
	*/
	@Override
	public InitialParameters getInitialParameters() throws Exception {
		...
		try {
			...
			Log.println (Log.ERROR, " ()", "Error while creating initial dialog: " + className, exc);
			throw exc;
		}
		...
		if (dlg.isValidDialog ()) {
			...
			if (ret == null) {
				throw new Exception("cannot get initial parameters");
			}
		}
	...
	/**	Return dialog to set evolution parameters 
	*	Used by default package.gui.PrefixEvolutionDialog
	*/
	@Override
	public EvolutionParameters getEvolutionParameters(Step stp) throws Exception {
		...
		if (dlg.isValidDialog ()) {
			...
			if (ret == null) {
				throw new Exception("cannot get evolution parameters");
			}
	...
	*/
	protected InitialParameters autoInitialParameters() throws Exception {
		...
		AutoDialog<InitialParameters> dlg = new AutoDialog<InitialParameters>(param, 
				new ObjectValidator() {
	...
	*	@param className
	*/
	protected EvolutionParameters autoEvolutionParameters(String className, Step stp) throws Exception {
		...
		try {
			...
			try {
				...
				Log.println (Log.ERROR, " ()", "Error while creating evolution parameters: " + className, e);
				throw exc;
			}
		}
	...
	/**	Return sketch linker instance
	*	Used by default package.sketch.PrefixSketchLinker
	*	May return null if the module is not sketch compatible
	*/
	@Override
