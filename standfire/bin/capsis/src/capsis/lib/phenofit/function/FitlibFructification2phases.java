...
//			boolean date2IsSet = (pheno.getDate2() > 0);
			...
			if (prevPhase <= 3) {
				...
				if (prevPhase == 3 && prevState3 >= 1) {
					...
					// ic 04/07/16 state of first day of phase 4 was not calculated
					double prevSum2 = 0;
...
//				if (!pheno.isSetDate2 () && year == pheno.birthYear + 1 && day >= nbDays) {
				...
				if (prevSum2 < matmoy - 3 * sigma) {
					...
					sum2 = wang.execute(dayClim.tmp)
							* (1d / (1 + Math.exp(-10 * (pheno.leafIndex - pfe50))))
							* loc5.droughtIndex[day-1] + prevSum2; // ic 04/07/16 we come back to the same droughtIndex as for Phenofit4 
							//* loc5.rew[day]+ prevSum2; // ic 13.4.2016 we use a water stress index calculated in the EndOfWaterBudget in Fit5Model L 480
							//* loc5.rew[day-1]+ prevSum2;// ic 01/07/16 this index is calculated after fruit maturation so we use the value of the preceding day						 
				} else {
...
//					}
	...
	public double execute(FitlibLocation loc, FitlibLocationClimate locClim, Fit4Phenology pheno, int year, int date0,
		...
		try {
			...
			int day = date0; // 06/03/2015 checked with IC //we deleted -1 in order to begin at 115 and not at 114
