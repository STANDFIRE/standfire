...
/**
 * The Phenofit FitlibFixedFructification function.
 * 
 * @author Isabelle Chuine, Yassine Motie - February 2015
 */
public class FitlibFixedFructification extends FitlibFunction2Phases implements FitlibPhelibFunctions {

	private double fruitGrowthInitDate;
	private double fruitMaturationDate;

	/**
	 * Constructor
	 */
	public FitlibFixedFructification(double fruitGrowthInitDate, double fruitMaturationDate) {
		this.fruitGrowthInitDate = fruitGrowthInitDate;
		this.fruitMaturationDate = fruitMaturationDate;
	}

	/**
	 * Constructor 2, parses a String, e.g. FixedFructification(7.2;145)
	 */
	public FitlibFixedFructification(String str) throws Exception {
		try {
			String s = str.replace("FixedFructification(", "");
			s = s.replace(")", "");
			s = s.trim();
			StringTokenizer st = new StringTokenizer(s, ";");
			fruitGrowthInitDate = Double.parseDouble(st.nextToken().trim());
			fruitMaturationDate = Double.parseDouble(st.nextToken().trim());
		} catch (Exception e) {
			throw new Exception("FitlibFixedFructification: could not parse this function: " + str, e);
		}
	}

	public String getName() {
		return "FixedFructification";
	}

	public String getExpectedParams() {
		return "dateMatDormancyBreak,dateFixed";
	}
	...
	public double executeDaily(FitlibLocation loc, FitlibLocationClimate locClim, int year, int day,
		...
		try {
			...
			if (prevPhase <= 3) {
				...
				// Does phase 4 start ?
				if (fit4Today > fruitGrowthInitDate) {
...
//					}
					...
					memory.intermediateState = (pheno.getPhenofit4Date(year, day) - pheno.getFloweringDate())
							/ (fruitGrowthInitDate - pheno.getFloweringDate());
...
//				if (year == pheno.birthYear + 1 && day >= nbDays) {
				...
				memory.state = (pheno.getPhenofit4Date(year, day) - pheno.getDate1())
						/ (fruitMaturationDate - pheno.getDate1());

				// Set date2
				if (fit4Today > fruitMaturationDate) {

					pheno.setDate2(year, fruitMaturationDate);
...
//					memory.resetDevelopmentStates();
			...
			Log.println(Log.ERROR, "FitlibFixedFructification.execute ()", "Error in FitlibFixedFructification", e);
			throw new Exception("Error in FitlibFixedFructification", e);
		}
	...
	public double execute(FitlibLocation loc, FitlibLocationClimate locClim, Fit4Phenology pheno, int year, int date0,
		...
		try {
			...
			pheno.setDate1(year, fruitGrowthInitDate);
	...
	public String toString() {
		return "FixedFructification(" + fruitGrowthInitDate + ";" + fruitMaturationDate + ")";
	}
