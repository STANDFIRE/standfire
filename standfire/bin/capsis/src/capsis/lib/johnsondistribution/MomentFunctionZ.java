...
*/
public class MomentFunctionZ implements IntegralFunction {
	private double dmed;
	private double csi;
	...
	/**	This function will be used for the evaluation of the Ipsilon moments.
	*/
	public MomentFunctionZ (double dmed, double csi, double initialLambda, double initialDelta, int order) {
		this.dmed = dmed;
		this.csi = csi;
	...
	*/
	public double function (double z) {
		return Math.exp (-0.5 * z * z) / Math.pow ((1 + Math.exp ((gamma - z) / initialDelta)), order);
	}
