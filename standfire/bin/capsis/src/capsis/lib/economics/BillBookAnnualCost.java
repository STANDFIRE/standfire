...
/**	An annual cost line in the BillBook.
*	@author O. Pain - may 2008
*/
public class BillBookAnnualCost extends BillBookLine {
	...
	public BillBookAnnualCost (AnnualCost cost, GScene stand, int year) {
		this.stand = stand;
		
		this.billBookRotationOrder = stand.getDate ();
		this.billBookYear = year;
		if (cost.isCost ()) {
			this.billBookOperation = Translator.swap ("BillBookAnnualCost.annualCost");
		} else {
			this.billBookOperation = Translator.swap ("BillBookAnnualCost.annualIncome");
		}
