...
public class HarvestingLortz extends WorkingProcess {

  private String name;
	private boolean LeavesOn; // km
  private double efficiency; // ratio
  private double walkingPerf; // mn/cm
	private double acquirePerf; // mn/cm
	private double fellingPerf; // mn/cm
  private double limbAndTopPerf; // mn/cm
  ...
  private double humanWorkingDayDuration_h;
    ...
    this.walkingPerf = walkingPerf;
		this.acquirePerf = acquirePerf;
		this.fellingPerf = fellingPerf;
    this.limbAndTopPerf = limbAndTopPerf;
    ...
    double humanWorkingDayDuration_h = doubleValue (st.nextToken ());
		...
		return new HarvestingLortz (name, LeavesOn, efficiency, walkingPerf, acquirePerf, fellingPerf, limbAndTopPerf, hourlyConsumption, machineWheight, machineLifetime, machineToHumanTimeRatio,	machineCarrierNeeded, stemMarketDestination, branchesLogging, branchesMarketDestination, smallWoodMarketDestination, meanCarConsumption_kWperkm, meanCarrierConsumption_kWperkm, humanWorkingDayDuration_h, contractorDistance_km);
	}
    ...
    double engineEfficiency = 0.35;
		...
		double walkingTime_h = walkingPerf * Math.pow (output.Dg / 2.54, 0.591) * output.treeNumber / 60d ;
		
    double acquireTime_h = acquirePerf * Math.pow (output.Dg / 2.54, 0.2) * output.treeNumber / 60d ;
		
    double cuttingTime_h = fellingPerf * Math.pow (output.Dg / 2.54, 0.937) * output.treeNumber / 60d ;
    
    double limbAndTopTime_h = limbAndTopPerf * Math.pow (output.Dg / 2.54, 2.129) * output.treeNumber / 60d ;
		
		double harvestingLortzTime_h = walkingTime_h + acquireTime_h + cuttingTime_h + limbAndTopTime_h ;
		
    double fuelConsumption = hourlyConsumption * harvestingLortzTime_h ;
    ...
    double lcConsumption = lcConsumption_perh * harvestingLortzTime_h;
    
    // 4. Logistics consumption
      // a. operatorTravelConsumption
      double humanProductiveWorkTime = harvestingLortzTime_h * machineToHumanTimeRatio;
    ...
    double processConsumption = fuelConsumption + oilConsumption + lcConsumption + logisticsConsumption;
		...
		output.machineWorkTime = harvestingLortzTime_h;
  ...
  public void energySpread (Resource r, double part) {
	...
	public String toString () {
		return "HarvestingLortz" 
        + "name :" + name 
        + " LeavesOn:" + LeavesOn 
        + " efficiency:" + efficiency
        + " walkingPerf:" + walkingPerf 
				+ " acquirePerf:" + acquirePerf
				+ " fellingPerf:" + fellingPerf
        + " limbAndTopPerf:" + limbAndTopPerf
