...
/**	PiecePropertyMeasurer : class used from PieceUtil to compute
...
*/
public abstract class PiecePropertyMeasurer {
	...
	// Constructor :
	public PiecePropertyMeasurer (GPiece piece) {
		this.piece = piece;
	}
	...
	*/
	...
	public double getMeasure_Total () {
		double integral = getIntegral (new DiscMeasurer () {
			public double getSurfacicMeasure (GPieceDisc disc) {
				...
				return PiecePropertyMeasurer.this.getSurfacicMeasure (
	...
	public double getMeasure_KnottyHeartWood () {
		double integral = getIntegral (new DiscMeasurer () {
			public double getSurfacicMeasure (GPieceDisc disc) {
				int cambialAgeMin = Math.max(disc.getNbSapWoodRings(), disc.getNbClearKnotRings()) + 1;
				int cambialAgeMax = disc.getNbWoodRings();
				return getSurfacicMeasure_Verified (
						disc, cambialAgeMin, cambialAgeMax);
			}
	...
	private double getIntegral (DiscMeasurer dm) {
		...
		for (Object o : piece.getDiscs()) {
			...
			if (firstDisc) {
				...
				System.out.println ("PiecePropertyMeasurer.getIntegral : discs not sorted !");
