...
/**	FgBeechDiscMassProvider : DiscMassProvider for Fagacees beech
*
*	@author F. Mothe - august 2008
*/
public class FgBeechDiscMassProvider implements DiscMassProvider {
	private static final long serialVersionUID = 20080901L;	// avoid java warning

	// TODO : keep the year into discs !
	private int getYear_TEMPO (int cambialAge) {
		// cambialAge = 0 => bark
		return 2000 + 1 - Math.max (cambialAge, 1);
	}
		...
		int cambialAgeMin, int cambialAgeMax)
	{
		...
		for (int cambialAge = cambialAgeMin; cambialAge <= cambialAgeMax; cambialAge++) {
			...
			if (cambialAge == 0) {
				// Bark :
				den_kgpm3 = FgCarbonCalculator.getBeechBarkBasicDensity_kgpm3();
			} else {
				...
				if (pithAge <= 0) {
					...
					// Wood :
					double rw_mm = R2 - R1;
					int year = getYear_TEMPO (cambialAge);
					den_kgpm3 = FgCarbonCalculator.getBeechWoodBasicDensity_kgpm3(
							R1, 
							pithAge, 
							year, 
							rw_mm);
