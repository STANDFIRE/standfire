...
package capsis.extension.treelogger.geolog.fagaceeslogging;
...
/**	FagLogOptionDialog : dialog box for FagaceesLogging options
*
*	@author F. Mothe - january 2006
*/

public class FagLogOptionDialog extends AmapDialog
		implements ActionListener {

	private static final long serialVersionUID = 20060918L;	// avoid java warning
	...
	/**	Default constructor.
	*/
	public FagLogOptionDialog (Starter starter) {
		super ();
		// System.out.println ("FagLogOptionDialog.FagLogOptionDialog ()");
		initialise (starter);
	}

	/**	Constructor for testing the dialog.
	*/
	public FagLogOptionDialog (JFrame frame, Starter starter) {
		super (frame);
		initialise (starter);
	}
		...
		try {
			...
			for (int m = 0; m<starter.models.size (); ++m) {
				slopes_ [m] = DiaUtil.checkedDoubleValue (slopes.get (m).getText (),
						"FagLogOptionDialog.slope", true, false, false);
				intercepts_ [m] = DiaUtil.checkedDoubleValue (intercepts.get (m).getText (),
						"FagLogOptionDialog.intercepts", true, false, false);
			}
			...
			System.out.println ("FagLogOptionDialog : okAction () Exception" + e);
		}
	...
	private void createUI () {
		...
		for (int m = 0; m<starter.models.size (); ++m) {
			...
			c1.add (DiaUtil.newTextAlone ("FagLogOptionDialog.model" + m,
				true, 50));
			c2.add (DiaUtil.newTextComponent ("FagLogOptionDialog.slope", 50,
				slopes.get (m), 4, starter.models.get (m).slope));
			c3.add (DiaUtil.newTextComponent ("FagLogOptionDialog.intercept", 50,
		...
		*/
		...
		setTitle (Translator.swap ("FagLogOptionDialog.title"));
		//
		setModal (true);
