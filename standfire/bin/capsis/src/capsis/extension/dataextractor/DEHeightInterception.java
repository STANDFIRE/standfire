...
/**
 * Tree height energy and potential energy scatterplot.
 *
 * @author B. Courbaud - November 2001
 */
public class DEHeightInterception extends DETimeG {
	private Vector labels;

	static {
		Translator.addBundle("capsis.extension.dataextractor.DEHeightInterception");
	}
	 ...
	 */
	public DEHeightInterception () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public DEHeightInterception (GenericExtensionStarter s) {
		//~ this (s.getStep ());
	//~ }
	...
	//~ /**
	 ...
	 //~ */
	//~ protected DEHeightInterception (Step stp) {
		super (s);
		labels = new Vector ();
	}
	...
	public boolean matchWith (Object referent) {
		...
		try {
			...
			Log.println (Log.ERROR, "DEHeightInterception.matchWith ()", "Error in matchWith () (returned false)", e);
			return false;
		}
		return true;
	}
	...
	public boolean doExtraction () {
		...
		//System.out.println ("DEHeightInterception : extraction being made");
		...
		try {
			...
			while (trees.hasNext ()) {
				...
				c1.add (new Double (h));
				c2.add (new Double (energy));
				c3.add (new Double (potEnergy));
			}
	...
	//		curves.add (c3);
			...
			Log.println (Log.ERROR, "DEHeightInterception.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DEHeightInterception");
	}
	...
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		v.add (Translator.swap ("DEHeightInterception.xLabel"));
		v.add (Translator.swap ("DEHeightInterception.yLabel"));
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DEHeightInterception.description");}
