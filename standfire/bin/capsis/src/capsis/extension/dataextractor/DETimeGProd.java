...
import capsis.util.methodprovider.ProdAliveGProvider;
import capsis.util.methodprovider.ProdGProvider;

/**	Production of Basal area ("Grundfläche" (G), "surface terrière" en allemand) over Time(/age) or over Hdom.
* 		including dead trees or not
*   With possibilty to view annual increment
*	@author Ph. Dreyfus - december 2008
*/
public class DETimeGProd extends PaleoDataExtractor implements DFCurves {
	protected Vector curves;
	protected MethodProvider methodProvider;

	private boolean availableHdom;
	private boolean availableAliveGProd;

	static {
		Translator.addBundle("capsis.extension.dataextractor.DETimeGProd");
	}
	...
	*/
	public DETimeGProd (GenericExtensionStarter s) {
		super (s);
		try {
			curves = new Vector ();
			...
			setPropertyEnabled ("onlyAliveG", availableAliveGProd); // PhD 2008-12-19
		} catch (Exception e) {
			Log.println (Log.ERROR, "DETimeGProd.c ()", "Exception occured while object construction : ", e);
		}
	}
	...
	*/
	public boolean matchWith (Object referent) {
		try {
			...
			if (!(mp instanceof ProdGProvider)) {return false;}

		} catch (Exception e) {
			Log.println (Log.ERROR, "DETimeGProd.matchWith ()", "Error in matchWith () (returned false)", e);
			return false;
		}
	...
	private void checkMethodProvider () {
		...
		if (methodProvider instanceof ProdAliveGProvider) {availableAliveGProd = true;}
	...
	*/
	public void setConfigProperties () {
		...
		addBooleanProperty ("onlyAliveG"); // PhD 2008-12-19
	}
	...
	public boolean doExtraction () {
		...
		try {
			...
			if (i.hasNext ()) { // if a least one step (... !)
				...
				if (isSet ("onlyAliveG") && (methodProvider instanceof ProdAliveGProvider)) { // PhD 2008-12-19
					read = ((ProdAliveGProvider) methodProvider).getProdAliveG (stand, trees) * coefHa;
				} else {
					read = ((ProdGProvider) methodProvider).getProdG (stand, trees) * coefHa;	// fc - 24.3.2004
				}
			...
			Log.println (Log.ERROR, "DETimeGProd.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	*/
	public String getName () {
		return getNamePrefix ()+Translator.swap ("DETimeGProd");
	}
	...
	*/
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		if (isSet ("xIsHdom")) { // PhD 2008-12-19
			v.add (Translator.swap ("DETimeGProd.xLabelHdom"));
		} else {
			v.add (Translator.swap ("DETimeGProd.xLabelDate"));
		}
		/*if (settings.perHa) {
			v.add (Translator.swap ("DETimeGProd.yLabel")+" (ha)");
		} else {
			v.add (Translator.swap ("DETimeGProd.yLabel"));
		}*/
		String yLab;
		yLab = Translator.swap ("DETimeGProd.yLabel");
		if (isSet ("onlyAliveG")) { // PhD 2008-12-19
			yLab = Translator.swap ("DETimeGProd.yLabelAlive");
		} else {
			yLab = Translator.swap ("DETimeGProd.yLabel");
		}
		if (settings.perHa) {
	...
	*/
	...
	/**	From Extension interface.
	*/
	public String getDescription () {return Translator.swap ("DETimeGProd.description");}
