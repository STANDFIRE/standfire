...
//	protected Vector labels;
	...
	//~ /**
	 ...
	 //~ */
	//~ protected DETimeDgByHdom (Step stp) {
		super (s);
		try {
			curves = new Vector ();
//			labels = new Vector ();
			...
			Log.println (Log.ERROR, "DETimeDgByHdom.c ()", "Exception occured while object construction : ", e);
		}
	}
	...
	public boolean doExtraction () {
		...
		try {
			...
			for (Iterator i = steps.iterator (); i.hasNext ();) {
				...
				double Dg = ((DgProvider) methodProvider).getDg (stand, trees); // kept in cm, so Dg/Hdom is in cm/m;
				double DgByHdom = 0d;
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DETimeDgByHdom");
	}
...
*/
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DETimeDgByHdom.description");}
