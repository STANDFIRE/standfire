...
/**
 * Numbers of trees per height/diameter classes,
 * based on DEDbhClassN.java.
 * H is reduced of 1.30m to have realistic H/D for small trees.
 *
 * @author B. Courbaud - 05/07/2001
 */
public class DESlendernessClassN extends DETimeG {
// fc - 30.4.2003 - added classWidth / minThreshold properties
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.DESlendernessClassN");
	}
	 ...
	 */
	public DESlendernessClassN () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public DESlendernessClassN (GenericExtensionStarter s) {
		super (s);
	...
	public boolean matchWith (Object referent) {
		try {
			...
			Log.println (Log.ERROR, "DESlendernessClassN.matchWith ()", "Error in matchWith () (returned false)", e);
			return false;
		}
	...
	public boolean doExtraction () {
		...
		//System.out.println ("DESlendernessClassN : extraction being made");
		...
		try {
			...
			// Create output data
//~ System.out.println ();
//~ System.out.println ("DESlendernessClassN ---");
			while (trees.hasNext ()) {
...
// bug correction : some maid trees may have number == 0 -> ignore them - tl 12/09/2005
				...
				if (t.getDbh () <= 0) continue; // PhD 2009-06-12

			//	double slenderness = 100*(t.getHeight ()-1.3) / t.getDbh (); //Height is reduced of 1.30 m to have realistic H/D for small trees
				double slenderness = 100*t.getHeight () / t.getDbh (); // same calculation than ForEole
//~ System.out.println ("tree_"+t.getId ()+" d="+t.getDbh ()+" h="+t.getHeight ()+" slenderness="+slenderness);
				if (slenderness < minThreshold) {continue;}	// fc - 30.4.2003
				int category = (int) ((slenderness-shift) / classWidth);
...
//~ System.out.println ();
			...
			Log.println (Log.ERROR, "DESlendernessClassN.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DESlendernessClassN");
	}
	...
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		v.add (Translator.swap ("DESlendernessClassN.xLabel"));
		if (settings.perHa) {
			v.add (Translator.swap ("DESlendernessClassN.yLabel")+" (ha)");
		} else {
			v.add (Translator.swap ("DESlendernessClassN.yLabel"));
		}
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DESlendernessClassN.description");}
