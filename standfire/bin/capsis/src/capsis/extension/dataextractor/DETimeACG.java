...
/**
 * Current Basal area increment ("Grundflache" (G), "surface terri�re") versus Date.
 *
 * @author L. Saint-André - august 2002
 */
public class DETimeACG extends PaleoDataExtractor implements DFCurves {	// MultiConfigurable goes up to DataExtractor
	...
	//~ /**
	 ...
	 //~ */
	//~ protected DETimeACG (Step stp) {
		super (s);
		try {
			curves = new Vector ();
			// MOVED TO SUPERCLASS retrieveSettings ();
		} catch (Exception e) {
			Log.println (Log.ERROR, "DETimeACG.c ()", "Exception occured while object construction : ", e);
		}
	}
	...
	public boolean matchWith (Object referent) {
		try {
			...
			if (!(mp instanceof ACGProvider)) {return false;}
	...
	public boolean doExtraction () {
		...
		try {
			...
			if (!i.hasNext ()) {throw new Exception ("DETimeACG needs at least two steps");}	// fc - 9.4.2004
			Step s = (Step) i.next ();
			...
			do {
				...
				double ACG = ((ACGProvider) methodProvider).getACG (
						stand, trees, previousStand, previousTrees) * coefHa;
	...
	public String getName () {
		return getNamePrefix ()+Translator.swap ("DETimeACG");
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DETimeACG.description");}
