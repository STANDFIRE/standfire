...
/**
 * Numbers of trees per diameter increment classes.
 *
 * @author B. Courbaud - May 2000
 */
public class DEDbhIncrementClassN extends DETimeG {
// fc - 30.4.2003 - added classWidth / minThreshold properties
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.DEDbhIncrementClassN");
	}
	 ...
	 */
	public DEDbhIncrementClassN () {}


	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public DEDbhIncrementClassN (GenericExtensionStarter s) {
		//~ this (s.getStep ());
	//~ }
	...
	//~ /**
	 ...
	 //~ */
	//~ protected DEDbhIncrementClassN (Step stp) {
		super (s);
		labels = new Vector ();
	...
	public boolean matchWith (Object referent) {
		try {
			...
			// fc - 22.8.2006 - if (t instanceof Numberable) {return false;}
			if (!(t instanceof DbhIncrementProvider)) {return false;}

		} catch (Exception e) {
			Log.println (Log.ERROR, "DEDbhIncrementClassN.matchWith ()", "Error in matchWith () (returned false)", e);
			return false;
		}
	...
	public void setConfigProperties () {
		...
		addDoubleProperty ("classWidthInMm", 1d);
		addDoubleProperty ("minThresholdInMm", 0d);
	...
	public boolean doExtraction () {
		...
		//System.out.println ("DEDbhIncrementClassN : extraction being made");
		...
		try {
			...
			double minThreshold = getDoubleProperty ("minThresholdInMm");
			double classWidth = getDoubleProperty ("classWidthInMm");
			...
			// Create output data
			while (trees.hasNext ()) {
				DbhIncrementProvider t = (DbhIncrementProvider) trees.next ();
				double dD = t.getDbhIncrement ()*10;		// dbhIncrement : mm because class width must be an integer

				if (dD < minThreshold) {continue;}	// fc - 30.4.2003
				
				int category = (int) ((dD-shift) / classWidth);
			...
			Log.println (Log.ERROR, "DEDbhIncrementClassN.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DEDbhIncrementClassN");
	}
	...
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		v.add (Translator.swap ("DEDbhIncrementClassN.xLabel"));
		if (settings.perHa) {
			v.add (Translator.swap ("DEDbhIncrementClassN.yLabel")+" (ha)");
		} else {
			v.add (Translator.swap ("DEDbhIncrementClassN.yLabel"));
		}
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DEDbhIncrementClassN.description");}
