...
public class DETimeInterceptionIndex extends DETimeG {
	...
	//~ /**
	 ...
	 //~ */
	//~ protected DETimeInterceptionIndex (Step stp) {
		super (s);
		try {
			curves = new Vector ();
			labels = new Vector ();
			...
			Log.println (Log.ERROR, "DETimeInterceptionIndex.c ()", "Exception occured while object construction : ", e);
		}
	}
	...
	public boolean matchWith (Object referent) {
		try {
			...
			if (!(mp instanceof DominantInterceptionIndexProvider)) {return false;}
	...
	public boolean doExtraction () {
		...
		try {
			...
			for (Iterator i = steps.iterator (); i.hasNext ();) {
				...
				c2.add (new Double (((MeanInterceptionIndexProvider) methodProvider).
						getMeanInterceptionIndex (stand, trees)));
				c3.add (new Double (((DominantInterceptionIndexProvider) methodProvider).
						getDominantInterceptionIndex (stand, trees)));
				...
				for (Iterator ids = treeIds.iterator (); ids.hasNext ();) {
					...
					if (t == null) {
						...
						cy[n].add (new Double (((TreeInterceptionIndexProvider) methodProvider).getTreeInterceptionIndex (t)));
					}
					n++;
				}
			}
	...
	public String getName() {
		return Translator.swap ("DETimeInterceptionIndex");
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DETimeInterceptionIndex.description");}
