 ...
 *
 * @author J. Labonne - 24.09.2004 (fully suggested by the design of B.Courbaud DEVolumeClassN extractor).
 */
public class FishAgeClassN extends PaleoDataExtractor implements DFCurves {
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.FishAgeClassN");
	}
	 ...
	 */
	public FishAgeClassN () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public FishAgeClassN (GenericExtensionStarter s) {
		//~ super (s.getStep ());
		super (s);
		curves = new Vector ();
		labels = new Vector ();
	...
	public boolean matchWith (Object referent) {
		try {
			if (!(referent instanceof FishModel)) {return false;}
				...
				Log.println (Log.ERROR, "FishAgeClassN.matchWith ()",
	...
	public void setConfigProperties () {
		...
		addDoubleProperty ("classWidthInMonth", 1d);
		addDoubleProperty ("minThresholdInMonth", 0d);
	...
	public boolean doExtraction () {
		...
		try {
			...
			double minThreshold = getDoubleProperty ("minThresholdInMonth");
			double classWidth = getDoubleProperty ("classWidthInMonth");
			...
			Iterator j = fishes.iterator();		// fc - 15.9.2004
			while (j.hasNext ()) {
				GFish f = (GFish) j.next ();

				double a = (double) f.getAge();
				if (a < minThreshold) {continue;}	// fc - 10.4.2003

				int category = (int) ((a-shift) / classWidth);
			...
			Log.println (Log.ERROR, "FishAgeClassN.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		...
		// fc - 15.9.2004
		return getNamePrefix ()+Translator.swap ("FishAgeClassN");
	}
	...
	public List<String> getAxesNames () {  //has to be called here, as no inheritance from DETimeG.
		Vector v = new Vector ();
		v.add (Translator.swap ("FishAgeClassN.xLabel"));
		if (isSet ("frequency")) {v.add (Translator.swap ("BidForkLengthClassN.yLabel2"));
		} else {v.add (Translator.swap ("BidForkLengthClassN.yLabel"));}
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("FishAgeClassN.description");}
