...
// fc - 30.4.2003 - added classWidth / minThreshold properties
	...
	//~ /**
	 ...
	 //~ */
	//~ protected DECrownBaseHeightDistrib (Step stp) {
		super (s);
		labels = new Vector ();
	...
	public boolean doExtraction () {
		...
		//System.out.println ("DECrownBaseHeightDistrib : extraction being made");
...
// bug correction : some maid trees may have number == 0 -> ignore them - tl 12/09/2005
				...
				if (d < minThreshold) {continue;}	// fc - 9.4.2003

				int category = (int) ((d-shift) / classWidth);

				Log.println("category "+category);

				double crownBaseHeight = (double) (((TreeCrownBaseHeightProvider) methodProvider).getTreeCrownBaseHeight (t) );

				Log.println("category "+category+" crownBaseHeight "+crownBaseHeight);

//TL 27/08/2014				tab1 [category] += crownBaseHeight;

//TL 27/08/2014				Log.println("category "+category+" tab1 "+ tab1 [category]);

				if (t instanceof Numberable) {
					double number = ((Numberable) t).getNumber ();	// fc - 22.8.2006 - Numberable returns double
					tabn [category] += number;	// ex: GMaidTree : the tree represents several
					//TL 27/08/2014
...
//~ System.out.println ("DECrownBaseHeightDistrib : minCat="+minCat+" maxCat="+maxCat);

			int anchor = 1;
			for (int i = minCat; i <= maxCat; i++) {
				//~ c1.add (new Integer (anchor++));
				c0.add (new Integer (i));	// fc - 7.8.2003 - bug correction (from PVallet)
//TL 27/08/2014
//				double numbers1 = tab1[i] / (double) tabnn [i];
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DECrownBaseHeightDistrib");
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DECrownBaseHeightDistrib.description");}
