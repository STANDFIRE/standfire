...
import mediterranea.model.MedModel;

/**
 * Number of fishes per survival classes.
 *
 * @author J. Labonne - august 2004
 */
public class FishSurvivalProbaClassN extends PaleoDataExtractor implements DFCurves {
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.FishSurvivalProbaClassN");
	}
	 ...
	 */
	public FishSurvivalProbaClassN () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public FishSurvivalProbaClassN (GenericExtensionStarter s) {
		//~ super (s.getStep ());
		super (s);
		curves = new Vector ();
		labels = new Vector ();
	...
	public boolean matchWith (Object referent) {
		try {
			...
			if ((referent instanceof MedModel)) {return false;}
			} catch (Exception e) {
				Log.println (Log.ERROR, "FishSurvivalProbaClassN.matchWith ()",
	...
	public void setConfigProperties () {
		...
		addDoubleProperty ("classWidth", 0.02d);
		addDoubleProperty ("minThreshold", 0d);
	...
	public boolean doExtraction () {
		...
		try {
			...
			while (j.hasNext ()) {
				...
				double sp = (double) f.getSurvivalProba();		// sp. no unit.
				if (sp < minThreshold) {continue;}	// fc - 10.4.2003

				int category = (int) ((sp-shift) / classWidth);
				//int category = (int) ((1-shift) / classWidth);
			...
			Log.println (Log.ERROR, "FishSurvivalProbaClassN.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		...
		// fc - 15.9.2004
		return getNamePrefix ()+Translator.swap ("FishSurvivalProbaClassN");
	}
	...
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		v.add (Translator.swap ("FishSurvivalProbaClassN.xLabel"));
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("FishSurvivalProbaClassN.description");}
