 ...
 */
public class ReachOrderClassN extends PaleoDataExtractor implements DFCurves {
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.ReachOrderClassN");
	}
	 ...
	 */
	public ReachOrderClassN () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public ReachOrderClassN (GenericExtensionStarter s) {
		//~ super (s.getStep ());
		super (s);
		curves = new Vector ();
		labels = new Vector ();
	...
	public boolean matchWith (Object referent) {
		try {
			if (!(referent instanceof FishModel) ) {return false;}
				...
				Log.println (Log.ERROR, "ReachOrderClassN.matchWith ()",
	...
	public boolean doExtraction () {
		...
		try {
			...
			Collection reaches = doFilter (stand, Group.REACH);	// fc - 16.9.2004
			...
			//~ for (Iterator k = refReaches.iterator (); k.hasNext ();) {
			for (Iterator k = reaches.iterator (); k.hasNext ();) {		// fc - 16.9.2004
				GReach refReach = (GReach) k.next ();

				int o = (int) refReach.getOrder();		//

				if (o < minThreshold) {continue;}	// fc - 10.4.2003

				int category = (int) ((o-shift) / classWidth);
			...
			Log.println (Log.ERROR, "ReachOrderClassN.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		...
		return getNamePrefix ()+Translator.swap ("ReachOrderClassN");	// fc - 16.9.2004
	}
	...
	public List<String> getAxesNames () {  // has to be defined here, is no more inherited from DETimeG.
		Vector v = new Vector ();
		v.add (Translator.swap ("ReachOrderClassN.xLabel"));
		if (isSet ("frequency")) {v.add (Translator.swap ("ReachOrderClassN.yLabel2"));
		} else {v.add (Translator.swap ("ReachOrderClassN.yLabel"));}
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("ReachOrderClassN.description");}
