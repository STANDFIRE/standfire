...
/**
 * Numbers of trees per height increment classes.
 *
 * @author B. Courbaud - May 2000
 */
public class DEHIncrementClassN extends DETimeG {
// fc - 30.4.2003 - added classWidth / minThreshold properties
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.DEHIncrementClassN");
	}
	 ...
	 */
	public DEHIncrementClassN () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public DEHIncrementClassN (GenericExtensionStarter s) {
		super (s);
		labels = new Vector ();
	...
	public boolean matchWith (Object referent) {
		try {
			...
			//~ if (t instanceof Numberable) {return false;}	// fc - 22.8.2006
			if (!(t instanceof HeightIncrementProvider)) {return false;}

		} catch (Exception e) {
			Log.println (Log.ERROR, "DEHIncrementClassN.matchWith ()", "Error in matchWith () (returned false)", e);
			return false;
		}
	...
	public boolean doExtraction () {
		...
		//System.out.println ("DEHIncrementClassN : extraction being made");
		...
		try {
			...
			// Create output data
			while (trees.hasNext ()) {
				HeightIncrementProvider t = (HeightIncrementProvider) trees.next ();
				double dH = t.getHeightIncrement ()*100;		// HIncrement : from m to cm because class width must be an integer
				if (dH < minThreshold) {continue;}	// fc - 30.4.2003
				int category = (int) ((dH-shift) / classWidth);
			...
			Log.println (Log.ERROR, "DEHIncrementClassN.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DEHIncrementClassN");
	}
	...
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		v.add (Translator.swap ("DEHIncrementClassN.xLabel"));
		if (settings.perHa) {
			v.add (Translator.swap ("DEHIncrementClassN.yLabel")+" (ha)");
		} else {
			v.add (Translator.swap ("DEHIncrementClassN.yLabel"));
		}
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DEHIncrementClassN.description");}
