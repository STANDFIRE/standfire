...
/**
 * Reach number per width classes.
 *
 * @author J. Labonne - august 2004 (fully suggested by the design of B.Courbaud DEVolumeClassN extractor).
 */
public class ReachMeanWidthClassN extends PaleoDataExtractor implements DFCurves {
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.ReachMeanWidthClassN");
	}
	 ...
	 */
	public ReachMeanWidthClassN () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public ReachMeanWidthClassN (GenericExtensionStarter s) {
		//~ super (s.getStep ());
		super (s);
		curves = new Vector ();
		labels = new Vector ();
	...
	public boolean matchWith (Object referent) {
		try {
			if (!(referent instanceof FishModel)) {return false;}
				...
				Log.println (Log.ERROR, "ReachMeanWidthClassN.matchWith ()",
	...
	public void setConfigProperties () {
		...
		addDoubleProperty ("classWidthInM", 1d);
	...
	public boolean doExtraction () {
		...
		try {
			...
			// Create output data
			//~ Collection refReaches = stand.getReachMap ().values ();	// fc - 17.9.2004
			for (Iterator k = reaches.iterator (); k.hasNext ();) {	// fc - 17.9.2004
				GReach refReach = (GReach) k.next ();

				double mw = (double) refReach.getMeanWidth();		//

				if (mw < minThreshold) {continue;}	// fc - 10.4.2003

				int category = (int) ((mw-shift) / classWidth);
			...
			Log.println (Log.ERROR, "ReachMeanWidthClassN.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		...
		return getNamePrefix ()+Translator.swap ("ReachMeanWidthClassN");
	}
	...
	public List<String> getAxesNames () {  // has to be defined here, is no more inherited from DETimeG.
		Vector v = new Vector ();
		v.add (Translator.swap ("ReachMeanWidthClassN.xLabel"));
		if (isSet ("frequency")) {v.add (Translator.swap ("ReachMeanWidthClassN.yLabel2"));
		} else {v.add (Translator.swap ("ReachMeanWidthClassN.yLabel"));}
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("ReachMeanWidthClassN.description");}
