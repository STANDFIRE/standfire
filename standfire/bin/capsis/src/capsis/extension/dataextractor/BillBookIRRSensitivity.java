...
/**	BillBook extension : sensitivity of IRR : internal rate or return ("TIR : Taux interne de rentabilité"in french).
*	@author F. de Coligny, O. Pain - march 2008
*/
public class BillBookIRRSensitivity extends PaleoDataExtractor implements DFColoredCurves {
	...
	static {
		Translator.addBundle("capsis.extension.dataextractor.BillBookIRRSensitivity");
	} 
	
	/**	Phantom constructor. 
	*	Only to ask for extension properties (authorName, version...).
	*/
	public BillBookIRRSensitivity () {}

	/**	Official constructor. It uses the standard Extension starter.
	*/
	public BillBookIRRSensitivity (GenericExtensionStarter s) {
		super (s);
		try {
			curves = new Vector ();
			labels = new Vector ();
			...
			Log.println (Log.ERROR, "BillBookIRRSensitivity.c ()", "Exception occured while object construction : ", e);
		}
	}
	...
	*/
	public boolean matchWith (Object referent) {
		try {
			...
			Log.println (Log.ERROR, "BillBookIRRSensitivity.matchWith ()", "Error in matchWith () (returned false)", e);
			return false;
		}
	}
	...
	*/
	...
	private void updateConfigProperties (Collection<String> soldProductNames, Collection<String> wpNames)	 {
		...
		addBooleanProperty ("includeAnnualAndVariableCosts",  Settings.getProperty ("includeAnnualAndVariableCosts", false));
	}
		...
		try {
			...
			for (String selectedName : selectedNames) {
				...
				for (double variation : variations) {
					...
					double x = variation;
					double IRR = FinancialTools.getInternalRateOfReturn (result.getYears (), result.getCashFlows ()) * 100;
			...
			Log.println (Log.ERROR, "BillBookIRRSensitivity.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	*/
	public String getName () {
		String name = getNamePrefix ()+Translator.swap ("BillBookIRRSensitivity");
	...
	*/
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		v.add (Translator.swap ("BillBookIRRSensitivity.xLabel"));
		//~ if (settings.perHa) {
			//~ v.add (Translator.swap ("BillBookIRRSensitivity.yLabel")+" (ha)");
		//~ } else {
			v.add (Translator.swap ("BillBookIRRSensitivity.yLabel"));
		//~ }
		return v;
	}
	...
	*/
	...
	/**	From Extension interface.
	*/
	public String getDescription () {return Translator.swap ("BillBookIRRSensitivity.description");}
