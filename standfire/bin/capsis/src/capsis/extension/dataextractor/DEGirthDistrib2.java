...
public class DEGirthDistrib2 extends DETimeG {
	...
	public boolean doExtraction () {
		...
		try {
			...
			// security
			// DEGirthDistrib2Settings mySettings = (DEGirthDistrib2Settings) getSettings ();
...
// bug correction : some maid trees may have number == 0 -> ignore them - tl 12/09/2005
				if ((t.getNumber()==0) && (t.getNumberOfDead()==0)) continue; // next iteration
				...
				tabb [category] += t.getNumber() + t.getNumberOfDead();		// Evolution : diff. = mortalité depuis 1 an ? depuis le step visible précédent ?  - A VERIFIER !!!
				if (category > maxCat) {
					maxCat = category;
				}
				if (category < minCat) {
			...
			for (int i = minCat; i <= maxCat; i++) {
				...
				int numbersdiff = (int) (tabb[i] * coefHa + 0.5);	// fc - 29.9.2004 : +0.5 (sp)
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DEGirthDistrib2");
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DEGirthDistrib2.description");}
