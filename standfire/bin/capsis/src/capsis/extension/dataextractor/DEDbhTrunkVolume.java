...
/**
 * Tree Volume Diameter Scatterplot.
 * (From DEdbhBiomass)
 * @author L. Saint-André - March 2004
 */
public class DEDbhTrunkVolume extends DETimeG {

	static {
		Translator.addBundle("capsis.extension.dataextractor.DEDbhTrunkVolume");
	}
	 ...
	 */
	public DEDbhTrunkVolume () {}

	/**
	 * Official constructor. It uses the standard Extension starter.
	 */
	public DEDbhTrunkVolume (GenericExtensionStarter s) {
		//~ this (s.getStep ());
	//~ }
	...
	//~ /**
	 ...
	 //~ */
	//~ protected DEDbhTrunkVolume (Step stp) {
		super (s);
	}
	...
	public boolean matchWith (Object referent) {
		try {
			...
			Log.println (Log.ERROR, "DEDbhTrunkVolume.matchWith ()", "Error in matchWith () (returned false)", e);
			return false;
		}
	...
	public boolean doExtraction () {
		...
		try {
			...
			while (trees.hasNext ()) {
				...
				double volume = treeStep.trunkVolume; //volume du tronc (m3)

				c1.add (new Double (dbh));
				c2.add (new Double (volume));
			}
			...
			Log.println (Log.ERROR, "DEDbhTrunkVolume.doExtraction ()", "Exception caught : ",exc);
			return false;
		}
	...
	public String getName() {
		return getNamePrefix ()+Translator.swap ("DEDbhTrunkVolume");
	}
	...
	public List<String> getAxesNames () {
		Vector v = new Vector ();
		v.add (Translator.swap ("DEDbhTrunkVolume.xLabel"));
		v.add (Translator.swap ("DEDbhTrunkVolume.yLabel"));
		return v;
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("DEDbhTrunkVolume.description");}
