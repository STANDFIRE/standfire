...
*/
public class LogTransporting extends PaleoWorkingProcess {

	static {
		Translator.addBundle("capsis.extension.workingprocess.LogTransporting");
	}
	...
	*/
	public LogTransporting () {}

	/**	Official constructor. Uses an ExtensionStarter.
	*/
	public LogTransporting (GenericExtensionStarter s) throws Exception {
		super (s);
	}
	...
	*/
	...
	public String getName () {
		return Translator.swap ("LogTransporting");
	}
	...
	/**	Extension interface */
	public String getDescription () {return Translator.swap ("LogTransporting.description");}

	public LogTransportingStarter getStarter () {return (LogTransportingStarter) starter;}
	...
	*/
	public Collection<String> getOutputProductNames () {
		Collection<String> r = new ArrayList<String> ();
		r.add (Product.TREE_MILL);
		r.add (Product.LOG_MILL);
		r.add (Product.BUNDLE_MILL);
		return r;
	}
	...
	*/
	public void execute () throws Exception {
		...
		// create Product.TREE_MILL
		if (starter.requestedOutputProductNames.contains (Product.TREE_MILL)) {
			...
			Product output = new Product (Product.TREE_MILL,
					input.getStand (),
			...
			addProduct (output);
			Log.println (Log.INFO, "LogTransporting.execute ()", "LogTransporting WP made: "+output);
		}

		// create Product.LOG_MILL
		if (starter.requestedOutputProductNames.contains (Product.LOG_MILL)) {
			Producer producer = this;
			Product input = starter.inputProduct;
			...
			Product output = new Product (Product.LOG_MILL,
					input.getStand (),
					input.getQty_ts (), // matter conservation with this working process
					input.getQty_tb (),
					input.getQty_m3 (),
		...
		// create Product.BUNDLE_MILL
		if (starter.requestedOutputProductNames.contains (Product.BUNDLE_MILL)) {
			Producer producer = this;
			Product input = starter.inputProduct;
			...
			Product output = new Product (Product.BUNDLE_MILL,
					input.getStand (),
					input.getQty_ts (), // matter conservation with this working process
					input.getQty_tb (),
					input.getQty_m3 (),
