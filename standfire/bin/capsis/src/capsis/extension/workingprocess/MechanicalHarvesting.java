...
*/
public class MechanicalHarvesting extends PaleoWorkingProcess {

	static {
		Translator.addBundle("capsis.extension.workingprocess.MechanicalHarvesting");
	}
	...
	*/
	public MechanicalHarvesting () {}

	/**	Official constructor. Uses an ExtensionStarter.
	*/
	public MechanicalHarvesting (GenericExtensionStarter s) throws Exception {
		super (s);
	}
	...
	*/
	...
	public String getName () {
		return Translator.swap ("MechanicalHarvesting");
	}
	...
	/**	Extension interface */
	public String getDescription () {return Translator.swap ("MechanicalHarvesting.description");}

	public MechanicalHarvestingStarter getStarter () {return (MechanicalHarvestingStarter) starter;}
	...
	*/
	public void execute () throws Exception {
		...
		// create Product.TREE
		if (starter.requestedOutputProductNames.contains (Product.TREE)) {
			...
			double qty_tb = species.getTED0biomass ((BillBookCompatible) input.getStand ());	//
			double qty_ts = species.convert (qty_tb, Product.GREEN_TON, Product.DRY_TON);
			double qty_m3 = species.convert (qty_tb, Product.GREEN_TON, Product.GREEN_TON);

			Product output = new Product (Product.TREE,
					input.getStand (),
					qty_ts,
					qty_tb,
					qty_m3,
			...
			addProduct (output);
			Log.println (Log.INFO, "MechanicalHarvesting.execute ()", "MechanicalHarvesting WP made: "+output);
		}

		// create Product.LOG
		if (starter.requestedOutputProductNames.contains (Product.LOG)) {
			Producer producer = this;
			Product input = starter.inputProduct;
			...
			double qty_tb = species.getTED7biomass ((BillBookCompatible) input.getStand ());	// m3
			double qty_ts = species.convert (qty_tb, Product.GREEN_TON, Product.DRY_TON);
			double qty_m3 = species.convert (qty_tb, Product.GREEN_TON, Product.GREEN_TON);

			Product output = new Product (Product.LOG,
					input.getStand (),
					qty_ts,
					qty_tb,
					qty_m3,
		...
		// create Product.CROWN
		if (starter.requestedOutputProductNames.contains (Product.CROWN)) {
			Producer producer = this;
			Product input = starter.inputProduct;
			...
			double qty_tb = species.getTED0biomass ((BillBookCompatible) input.getStand ())
					- species.getTED7biomass ((BillBookCompatible) input.getStand ());	// tb
			double qty_ts = species.convert (qty_tb, Product.GREEN_TON, Product.DRY_TON);
			double qty_m3 = species.convert (qty_tb, Product.GREEN_TON, Product.M3);

			Product output = new Product (Product.CROWN,
					input.getStand (),
					qty_ts,
					qty_tb,
					qty_m3,
