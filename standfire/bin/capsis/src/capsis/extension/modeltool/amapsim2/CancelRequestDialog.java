...
/**
 * CancelRequestDialog is a dialog box to enter parameters for the related request.
 * 
 * @author F. de Coligny - november 2002
 */
public class CancelRequestDialog extends AmapDialog implements ActionListener {
	...
	public CancelRequestDialog (CancelRequest request) {
		super ();
	...
	private void okAction () {
		...
		try {
			...
			MessageDialog.print (this, Translator.swap ("CancelRequestDialog.wrongSyntax"), e);
			return;
		}
		
		if (ids == null || ids.length == 0) {
			MessageDialog.print (this, Translator.swap ("CancelRequestDialog.wrongSyntax")+" ids="+ids);
			return;
		} else {
			try {
	...
	private void createUI () {
		...
		l1.add (new JWidthLabel (Translator.swap ("CancelRequestDialog.messageId")+" :", 120));
		messageId = new JTextField (10);
		l1.add (messageId);
		l1.addStrut0 ();
		
		LinePanel l2 = new LinePanel ();
		l2.add (new JLabel (Translator.swap ("CancelRequestDialog.correctSyntax")));
		l2.addGlue ();
		...
		setTitle (Translator.swap ("CancelRequestDialog"));
