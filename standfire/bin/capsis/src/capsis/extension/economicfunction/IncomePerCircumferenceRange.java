 ...
 */
public class IncomePerCircumferenceRange extends Income {

	protected SortedMap downLimitAndPrice;

	static {
		Translator.addBundle("capsis.extension.economicfunction.IncomePerCircumferenceRange");
	}
	 ...
	 */
	public IncomePerCircumferenceRange () {}

	/**
	 * Official constructor. Uses an ExtensionStarter.
	 */
	public IncomePerCircumferenceRange (GenericExtensionStarter s) throws Exception {
		try {
			...
			if (s instanceof IncomePerCircumferenceRangeStarter) {

				// 2. Script mode starter
				IncomePerCircumferenceRangeStarter p = (IncomePerCircumferenceRangeStarter) s;
				downLimitAndPrice = p.downLimitAndPrice;

			} else {
				Log.println ("Try to open IncomePerCircumferenceRangedialog");
				// 3. Interactive start
				IncomePerCircumferenceRangeDialog dlg = new IncomePerCircumferenceRangeDialog ();
			...
			if (! (exc instanceof CancelException)) {	// do not log if cancel
				Log.println (Log.ERROR, "IncomePerCircumferenceRange.c ()", exc.toString (), exc);
			}
			throw exc;
		}
	 ...
	 */
	public IncomePerCircumferenceRange (GenericExtensionStarter s, String stringParameters) throws Exception {
		try {
			stand = s.getScene ();	// stand is defined in EconomicFunction
			model = s.getModel ();	// model is defined in EconomicFunction
	 ...
	 * Computation of the expense using getTreeV function of model
	 */
	public double getResult () {
		...
		if (trees.isEmpty()){
			...
			for (Iterator i =  trees.iterator (); i.hasNext ();){
				...
				double tCirc = t.getDbh()*2*Math.PI; // tree circumference
				if (downLimitAndPrice.containsKey(tCirc)){
					tp = (Double) downLimitAndPrice.get(tCirc);
				} else {
					sousTable = downLimitAndPrice.headMap(tCirc);
			...
			total += tV*tp; 
			Log.println (Log.INFO,"IncomePerDiameterRange Volume ","Tree "+ t.getId()+ " Circunference:"+tCirc+" prix :"+ tp +"Volume:"+tV+ " cumul : "+total);
			}			
		return total;
		...
		SortedMap sousTable;
			try {
				...
				Log.println (Log.ERROR, "IncomePerCircumferenceRange.getResult ()",
			...
			try {
				for (Iterator i = tcf.getIds ().iterator (); i.hasNext ();) {
					...
					Double treeFatherCirc = new Double(treeFather.getDbh()*2*Math.PI);
					if (treeFather instanceof Numberable){
						...
						Log.println (Log.INFO, "IncomePerCircumferenceRange", "Non instance of Numerable");
					}
					p=new Double(0d);
					if (downLimitAndPrice.containsKey(treeFatherCirc)){
						p = (Double) downLimitAndPrice.get(treeFatherCirc);
					} else {
						sousTable = downLimitAndPrice.headMap(treeFatherCirc);
						if (!sousTable.isEmpty()){
							p = (Double) downLimitAndPrice.get(sousTable.lastKey());
							Log.println (Log.INFO, "IncomePerCircumferenceRange", id_tree+" Cicrcumference "+treeFatherCirc+" Price:"+ p);
						} else {
							p = (Double) downLimitAndPrice.get(downLimitAndPrice.firstKey());
							Log.println (Log.WARNING, "IncomePerCircumferenceRange", "subtable is null Affect lower price:"+p);
						}
					}

					total += V*p.doubleValue(); // voir comment faire la diff�rence avec volume apr�s coupe.
					Log.println (Log.INFO,"IncomePerCircumferenceRange Volume ", "Volume:"+V+" prix :"+ p + " cumul : "+total);
				}
				return total;		// V*p
			} catch (Exception e) {
				Log.println (Log.ERROR, "IncomePerCircumferenceRange.getResult ()", "Error while computing V*p", e);
				return 0d;
		}*/
	}
	...
	public String getFunctionParameters () {
		return getName ()
				+", "+Translator.swap ("IncomePerCircumferenceRangeDialog.valueList")+"="
	...
	*/
	public String getParametersList (){
		return Translator.swap ("IncomePerCircumferenceRangeDialog.valueList")+separator;
	}
	...
	public String getName () {
		return Translator.swap ("IncomePerCircumferenceRange");
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("IncomePerCircumferenceRange.description");}
