 ...
 */
public class IncomePerCircumferenceRangeDialog extends AmapDialog implements ActionListener {

	private SortedMap downCircumferenceAndPrice;
	...
	private static final int CIRCUMFERENCE = 0;
	private static final int PRICE = 1;


	public IncomePerCircumferenceRangeDialog () {
		super ();
		createUI ();
		setTitle (Translator.swap ("IncomePerCircumferenceRangeDialog"));
	...
	public SortedMap getValue () {
		downCircumferenceAndPrice = new TreeMap();
		TableModel t = table.getModel ();
		int nRow = t.getRowCount ();
		for (int i = 0; i < nRow; i++) {
			String Circumference = (String) t.getValueAt (i, CIRCUMFERENCE);
			String price = (String) t.getValueAt (i, PRICE);
			if (Check.isDouble(Circumference) && Check.isDouble(price)){
				//System.out.println ("IncomePerCircumferenceRangeData : "+ Circumference+";"+price);
				downCircumferenceAndPrice.put (new Double(Circumference),new Double(price));

			}
		}
		return downCircumferenceAndPrice;
	}
	...
	private void okAction () {
		...
		for (int i = 0; i < nRow; i++) {
			...
			String price = (String) t.getValueAt (i, PRICE);
			if (Check.isDouble(Circumference) && Check.isDouble(price)){
				//System.out.println ("IncomePerCircumferenceRangeData : "+ (new Double(t.getValueAt (i, Circumference).toString())+";"+new Double(t.getValueAt (i, PRICE).toString())));
				downCircumferenceAndPrice.put (new Double(Circumference),new Double(price));
			}
		}
		if (downCircumferenceAndPrice.isEmpty ()) {
			MessageDialog.print (this, Translator.swap ("IncomePerCircumferenceRangeDialog.youMustProvideData"));
			return;
		}
	...
	public void actionPerformed (ActionEvent evt) {
		...
		//if (fileNameTable2.getText()==null || fileNameTable2.getText().equals("") || !Check.isFile(fileNameTable2.getText())){
			...
			fileName=browseAction ( Translator.swap ("IncomePerCircumferenceRangeDialog.saveInATextFile"));
		//	}
			System.out.println ("fileName: "+fileName);
			try{
				saveTableInAFile (fileName);
			} catch (Exception e) {
					Log.println (Log.ERROR, "IncomePerCircumferenceRangeDialog.actionPerformed ().SAveInATextFile",
							"Error while saving table into text file due to exception ", e);
					MessageDialog.print (this, Translator.swap ("IncomePerCircumferenceRangeDialog.errorWhileSavingIntoTextFileTable2"));
			}
		...
		if (evt.getSource ().equals (loadTable)) {
			//if (fileNameTable2.getText()==null || fileNameTable2.getText().equals("")){
				fileName=browseAction (Translator.swap ("IncomePerCircumferenceRangeDialog.loadFromATextFile"));
			//}
			try{
				readTableInFile (fileName);
			} catch (Exception e) {
					Log.println (Log.ERROR, "IncomePerCircumferenceRangeDialog.actionPerformed ()",
							"Error while loading text file into table 2 due to exception ", e);
					MessageDialog.print (this, Translator.swap ("IncomePerCircumferenceRangeDialog.errorWhileLoadingFromTextFileTable"));
			}
			//hasbeenmodified = true;
		}
	}
	...
	private String browseAction (String label) {
		...
		*/
		public void saveTableInAFile (String fileName) throws Exception {
			...
			out.add("Circumference"+separator+"price");
			//downCircumferenceAndPrice = new TreeMap();
			TableModel t = table.getModel ();
			int nRow = t.getRowCount ();
			for (int i = 0; i < nRow; i++) {
				...
				if (Check.isDouble(Circumference) && Check.isDouble(price)){
					//System.out.println ("IncomePerCircumferenceRangeData : "+ Circumference+";"+price);
					out.add (Circumference+separator+price+separator);
				}
			}
			//out.add(emptyRow2);
			CommonEconFunctions.updateEconParametersFile (fileName, "IncomePerCircumferenceRangeDialog", "PriceTable", out);
	    }
	...
	*/
	public void readTableInFile (String fileName) throws Exception {
			...
			fileContent = CommonEconFunctions.getValueSubTagEconParametersFile(fileName, "IncomePerCircumferenceRangeDialog", "PriceTable");
			if  (fileContent.size()!=0){
				...
				Log.println (Log.WARNING, "IncomePerCircumferenceRangeDialog.loadingfromtext ()",
	...
	*/
	...
	private void createUI () {
		...
		columnNames.add (Translator.swap ("IncomePerCircumferenceRangeDialog.downCircumference"));
		columnNames.add (Translator.swap ("IncomePerCircumferenceRangeDialog.price"));
