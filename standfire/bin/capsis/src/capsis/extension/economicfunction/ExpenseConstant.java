...
/**
 * Associate a constant expense to an intervention
 *
 * @author C. Orazio - january 2003
 */
public class ExpenseConstant extends Expense {

	protected double value;

	static {
		Translator.addBundle("capsis.extension.economicfunction.ExpenseConstant");
	}
	 ...
	 */
	public ExpenseConstant (GenericExtensionStarter s) throws Exception {
		try {
			...
			if (s instanceof ExpenseConstantStarter) {

				// 2. Script mode starter
				ExpenseConstantStarter p = (ExpenseConstantStarter) s;
				value = p.value;

			} else {

				// 3. Interactive start
				ExpenseConstantDialog dlg = new ExpenseConstantDialog ();
			...
			if (! (exc instanceof CancelException)) {	// do not log if cancel
				Log.println (Log.ERROR, "ExpenseConstant.c ()", exc.toString (), exc);
			}
			throw exc;
		}
	 ...
	 */
	public ExpenseConstant (GenericExtensionStarter s, String stringParameters) throws Exception {
		try {
			stand = s.getScene ();	// stand is defined in EconomicFunction
			model = s.getModel ();	// model is defined in EconomicFunction
	...
	public String getFunctionParameters () {
		return getName ()+", "+Translator.swap ("ExpenseConstantDialog.value")+"="+value;
	}
	...
	*/
	public String getParametersList (){
		return Translator.swap ("ExpenseConstantDialog.value")+separator;
	}
	...
	public String getName () {
		return Translator.swap ("ExpenseConstant");
	}
	 ...
	 * From Extension interface.
	 */
	public String getDescription () {return Translator.swap ("ExpenseConstant.description");}
